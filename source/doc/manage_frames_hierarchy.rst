...
.. code-block:: python
    ...
    frame_binder = py3dframe.FrameBinder()

    frame_binder.add_frame(frame1, name='F1', link=None)
    frame_binder.add_frame(frame2, name='F2', link='F1')
    frame_binder.add_frame(frame3, name='F3', link=None)

The attribute `link` of the method `add_frame` is used to link the frame to another frame by its name.
...
.. code-block:: python
    ...
    point_S3 = [1, 1, 1]
    point_S2 = frame_binder.from_frame_to_frame(input_name='F3', output_name='F2', point=point_S3)
