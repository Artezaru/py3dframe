...
class Frame(object):
    ...
    def origin(self) -> numpy.ndarray:
        ...
        The origin is the coordinates of the frame in the world frame.
    ...
    def _inverse_conversion(self) -> (numpy.ndarray, numpy.ndarray):
        ...
        .. math::
            ...
            \mathbf{p}_{\text{local}} = \mathbf{R}^T (\mathbf{p}_{\text{world}} - \mathbf{O})
    ...
    def from_world_to_local(
        ...
        *, 
        ...
        # Convert the data to local coordinates
        local_data = numpy.dot(self.rotation_matrix.T, data)
    ...
    def from_local_to_world(
        ...
        *, 
        ...
        .. math::
            ...
            \mathbf{p}_{\text{world}} = \mathbf{R} \mathbf{p}_{\text{local}} + \mathbf{O}
        ...
        if point is not None:
            world_data = world_data + self.origin
    ...
    def compose(self, other: Frame) -> Frame:
        ...
        The current frame is define from the world to the local (1).

        .. math::

            \mathbf{p}_{\text{world}} = \mathbf{R_{(1)}} \mathbf{p}_{(1)} + \mathbf{O}_{(1)}

        The other frame is define from the local (1) to the local (2).

        .. math::

            \mathbf{p}_{(1)} = \mathbf{R_{(2)}} \mathbf{p}_{(2)} + \mathbf{O}_{(2)}

        The composition is define from the world to the local (2).

        .. math::

            \mathbf{p}_{\text{world}} = \mathbf{R_{(1)}} \mathbf{R_{(2)}} \mathbf{p}_{(2)} + \mathbf{R_{(1)}} \mathbf{O}_{(2)} + \mathbf{O}_{(1)}
    ...
    def __mul__(self, other: Frame) -> Frame:
        ...
        The self frame is define from the world to the local (1).
        The other frame is define from the local (1) to the local (2).
