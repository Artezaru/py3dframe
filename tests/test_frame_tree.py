...
def test_add_and_get_frame(frame_tree):
    ...
    assert frame_tree.parents["frame1"] is None, "frame1 should have no parent."
    assert frame_tree.parents["frame2"] == "frame1", "frame2 should have frame1 as parent."
    assert frame_tree.parents["frame3"] is None, "frame3 should have no parent."
...
def test_set_name(frame_tree):
    ...
    assert frame_tree.parents["frame2"] == "base", "Parent reference was not updated correctly after renaming."

def test_set_parent(frame_tree):
    assert frame_tree.parents["frame3"] is None, "frame3 should initially have no parent."
    frame_tree.set_parent("frame3", "frame1")
    assert frame_tree.parents["frame3"] == "frame1", "Setting frame3's parent to frame1 failed."
...
def test_remove_frame(frame_tree):
    ...
    assert frame_tree.parents["frame2"] is None, "frame2's parent should be set to None after removing its parent."
...
def test_get_globalcompose_frame(frame_tree):
    ...
    point_frame_1 = frame_tree["frame1"].from_global_to_frame(point=global_point)
    point_frame_2 = frame_tree["frame2"].from_global_to_frame(point=point_frame_1)
    composed = frame_tree.get_globalcompose_frame("frame2")
...
def test_get_compose_frame(frame_tree):
    composed = frame_tree.get_compose_frame(input_name="frame1", output_name="frame2")
