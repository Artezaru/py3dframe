...
def test_translate_local():
    ...
    translated_frame = translate(frame, dx=1.0, dy=2.0, dz=3.0, use_global=False, inplace=False)

    expected_origin_local = np.array([2.0, 4.0, 6.0]).reshape((3, 1))
    assert np.allclose(translated_frame.origin, expected_origin_local)


def test_translate_global():
    frame = create_test_frame()
    translated_frame = translate(frame, dx=1.0, dy=2.0, dz=3.0, use_global=True, inplace=False)

    # dx global -> dy for parent
    # dy global -> dz for parent
    # dz global -> dx for parent

    expected_delta_origin_local = np.array([3.0, 1.0, 2.0]).reshape((3, 1))
